...
def openUrlAndGetContent(url):
    """打开指定网址并获取页面的文本内容"""
    print("openUrlAndGetContent被调用")
    # 这里用伪代码模拟浏览器打开页面和内容提取的过程
    # 真实情况中，这需要使用例如Selenium或requests-html等库
    simulated_response = "This is a simulated response text from the website."
    return f"Retrieved content from {url}: {simulated_response}"
...
starting_tools = [
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "openUrlAndGetContent",
            "description": "打开指定网址并获取页面文本内容",
            "parameters": {
                "type": "object",
                "properties": {
                    "url": {
                        "type": "string",
                        "description": "完整的网址，例如：https://www.example.com",
                    },
                },
                "required": ["url"],
            },
...
def run_action(thread_id, run_id):
    ...
    for tool in run.required_action.submit_tool_outputs.tool_calls:
        ...
        elif tool.function.name == "openUrlAndGetContent":
            arguments = json.loads(tool.function.arguments)
            url = arguments["url"]
            name_info = openUrlAndGetContent(url)
            client.beta.threads.runs.submit_tool_outputs(
                thread_id=thread_id,
                run_id=run.id,
                tool_outputs=[
...
def main():
    ...
    if 1==1:
        ...
        user_message = "www.reddit.com的内容是什么"
